...
class PoiDataset(Dataset):
    ...
    def __init__(self, users, times, weeks, days, time_slots, hours, coords, locs, quadkeys, sequence_length, batch_size, split, usage, loc_count,
                 custom_seq_count):
        self.users = users
        self.locs = locs
        ...
        self.sequences_quadkeys = []
        self.sequences_times = []
        self.sequences_weeks = []
        self.sequences_days = []
        ...
        self.sequences_lbl_quadkeys = []
        self.sequences_lbl_times = []
        self.sequences_lbl_weeks = []
        self.sequences_lbl_days = []
        self.sequences_lbl_time_slots = []
        self.sequences_lbl_hours = []
        ...
        for i, loc in enumerate(locs):
            ...
            self.lbl_weeks.append(self.weeks[i][1:])
            self.lbl_days.append(self.days[i][1:])
            self.lbl_hours.append(self.hours[i][1:])
            self.lbl_coords.append(self.coords[i][1:])
            self.lbl_quadkeys.append(self.quadkeys[i][1:])

            self.quadkeys[i] = self.quadkeys[i][:-1]
            self.times[i] = self.times[i][:-1]
            self.time_slots[i] = self.time_slots[i][:-1]
            self.weeks[i] = self.weeks[i][:-1]
            self.days[i] = self.days[i][:-1]
            self.hours[i] = self.hours[i][:-1]
            self.coords[i] = self.coords[i][:-1]

        # split to training / test phase:
        for i, (time, week, day, time_slot, hour, coord, loc, quadkey, label, lbl_time, lbl_week, lbl_day, lbl_time_slot, lbl_hour, lbl_coord, lbl_quadkey) in enumerate(
                zip(self.times, self.weeks, self.days, self.time_slots, self.hours, self.coords, self.locs, self.quadkeys, self.labels, self.lbl_times, self.lbl_weeks, self.lbl_days,
                    self.lbl_time_slots, self.lbl_hours, self.lbl_coords, self.lbl_quadkeys)):
            train_thr = int(len(loc) * 0.8)
            if split == Split.TRAIN:
                self.locs[i] = loc[:train_thr]
                self.times[i] = time[:train_thr]
                self.weeks[i] = week[:train_thr]
                self.days[i] = day[:train_thr]
                self.time_slots[i] = time_slot[:train_thr]
                self.hours[i] = hour[:train_thr]
                self.coords[i] = coord[:train_thr]
                self.quadkeys[i] = quadkey[:train_thr]

                self.labels[i] = label[:train_thr]
                self.lbl_times[i] = lbl_time[:train_thr]
                self.lbl_weeks[i] = lbl_week[:train_thr]
                self.lbl_days[i] = lbl_day[:train_thr]
                self.lbl_time_slots[i] = lbl_time_slot[:train_thr]
                self.lbl_hours[i] = lbl_hour[:train_thr]
                self.lbl_coords[i] = lbl_coord[:train_thr]
                self.lbl_quadkeys[i] = lbl_quadkey[:train_thr]
        ...
        for i, (time, week, day, time_slot, hour, coord, loc, quadkey, label, lbl_time, lbl_week, lbl_day, lbl_time_slot, lbl_hour, lbl_coord, lbl_quadkey) in enumerate(
            ...
            for j in range(seq_count):
                ...
                seq_weeks.append(week[start:end])
                seq_days.append(day[start:end])
                seq_time_slots.append(time_slot[start:end])
                seq_hours.append(hour[start:end])
                seq_coords.append(coord[start:end])
                seq_quadkeys.append(quadkey[start:end])

                seq_lbls.append(label[start:end])
                seq_lbl_times.append(lbl_time[start:end])
                seq_lbl_weeks.append(lbl_week[start:end])
                seq_lbl_days.append(lbl_day[start:end])
                seq_lbl_time_slots.append(lbl_time_slot[start:end])
                seq_lbl_hours.append(lbl_hour[start:end])
                seq_lbl_coords.append(lbl_coord[start:end])
                seq_lbl_quadkeys.append(lbl_quadkey[start:end])

            self.sequences.append(seqs)
            self.sequences_times.append(seq_times)
            self.sequences_weeks.append(seq_weeks)
            self.sequences_days.append(seq_days)
            ...
            self.sequences_quadkeys.append(seq_quadkeys)

            self.sequences_labels.append(seq_lbls)
            self.sequences_lbl_times.append(seq_lbl_times)
            self.sequences_lbl_weeks.append(seq_lbl_weeks)
            self.sequences_lbl_days.append(seq_lbl_days)
            self.sequences_lbl_time_slots.append(seq_lbl_time_slots)
            self.sequences_lbl_hours.append(seq_lbl_hours)
            self.sequences_lbl_coords.append(seq_lbl_coords)
            self.sequences_lbl_quadkeys.append(seq_lbl_quadkeys)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            weeks.append(torch.tensor(self.sequences_weeks[i_user][j]))
            days.append(torch.tensor(self.sequences_days[i_user][j]))
            time_slots.append(torch.tensor(self.sequences_time_slots[i_user][j]))
            hours.append(torch.tensor(self.sequences_hours[i_user][j]))
            coords.append(torch.tensor(self.sequences_coords[i_user][j]))
            #print(self.sequences_quadkeys[i_user][j])
            quadkeys.append(torch.tensor(self.sequences_quadkeys[i_user][j]))

            lbls.append(torch.tensor(self.sequences_labels[i_user][j]))
            lbl_times.append(torch.tensor(self.sequences_lbl_times[i_user][j]))
            lbl_weeks.append(torch.tensor(self.sequences_lbl_weeks[i_user][j]))
            lbl_days.append(torch.tensor(self.sequences_lbl_days[i_user][j]))
            lbl_time_slots.append(torch.tensor(self.sequences_lbl_time_slots[i_user][j]))
            lbl_hours.append(torch.tensor(self.sequences_lbl_hours[i_user][j]))
            lbl_coords.append(torch.tensor(self.sequences_lbl_coords[i_user][j]))
            lbl_quadkeys.append(torch.tensor(self.sequences_lbl_quadkeys[i_user][j]))

            self.active_user_seq[i] += 1  # 如果用户i再次被选择为活跃用户时,j加1使得选择i的后续的子序列

        x = torch.stack(seqs, dim=1)
        t = torch.stack(times, dim=1)
        w = torch.stack(weeks, dim=1)
        d = torch.stack(days, dim=1)
        t_slot = torch.stack(time_slots, dim=1)
        h = torch.stack(hours, dim=1)
        s = torch.stack(coords, dim=1)
        q = torch.stack(quadkeys, dim=1)

        y = torch.stack(lbls, dim=1)
        y_t = torch.stack(lbl_times, dim=1)
        y_w = torch.stack(lbl_weeks, dim=1)
        y_d = torch.stack(lbl_days, dim=1)
        y_t_slot = torch.stack(lbl_time_slots, dim=1)
        y_h = torch.stack(lbl_hours, dim=1)
        y_s = torch.stack(lbl_coords, dim=1)
        y_q = torch.stack(lbl_quadkeys, dim=1)
        return x, t, w, d, t_slot, h, s, q, y, y_t, y_w, y_d, y_t_slot, y_h, y_s, y_q, reset_h, torch.tensor(self.active_users)
