...
class Evaluation_quad:
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            u_entropy_all = np.zeros(self.user_count)

            for i, (x, t, w, d, t_slot, hour, s, q, y, y_t, y_w, y_d, y_t_slot, y_hour, y_s, y_q, reset_h, active_users) in enumerate(self.dataloader):
                ...
                w = w.squeeze().to(self.setting.device)
                d = d.squeeze().to(self.setting.device)
                t_slot = t_slot.squeeze().to(self.setting.device)
                hour = hour.squeeze().to(self.setting.device)
                s = s.squeeze().to(self.setting.device)
                q = q.squeeze().to(self.setting.device)

                y = y.squeeze()
                y_t = y_t.squeeze().to(self.setting.device)
                y_w = y_w.squeeze().to(self.setting.device)
                y_d = y_d.squeeze().to(self.setting.device)
                y_t_slot = y_t_slot.squeeze().to(self.setting.device)
                y_hour = y_hour.squeeze().to(self.setting.device)
                y_s = y_s.squeeze().to(self.setting.device)
                y_q = y_q.squeeze()
                active_users = active_users.to(self.setting.device)

                # evaluate:
                out, q_out, h, location_topk, filter_result, retain_result = self.trainer.evaluate(x, q, t, w, d, t_slot, hour, s, y_t, y_t_slot, y_s, h, active_users)
                q_out[3] = q_out[3].transpose(0,1)
                for j in range(self.setting.batch_size):
                    ...
                    for k in range(len(y_j)):
                        ...
                        if t in r[:10]:
                            q_value = torch.softmax(q_out[3][j, k], dim=-1)
                            entropy = -torch.sum(q_value * torch.log(q_value + 1e-9), dim=-1)
                            u_entropy_all[active_users[j]] += entropy.item()
            ...
            log_string(self._log, 'Entropy: ' + formatter.format(entropy_all / iter_cnt))
            print('predictions:', iter_cnt)
