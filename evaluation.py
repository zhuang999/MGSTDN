    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for i, (x, t, w, d, t_slot, hour, s, q, y, y_t, y_w, y_d, y_t_slot, y_hour, y_s, y_q, reset_h, active_users) in enumerate(self.dataloader):
                ...
                y_q = y_q.squeeze().to(self.setting.device)
                ...
                for j in range(self.setting.batch_size):
                    ...
                    for k in range(len(y_j)):
                        ...
                        # # produce plot experiments data
                        # if t in r[:1]:
                        #     prob = F.softmax(o[k],dim=-1)
                        #     log_prob = F.log_softmax(o[k], dim=-1)
                        #     entropy = -torch.sum(prob * log_prob, dim=-1)
                        #     confidence = 1- (entropy / torch.log(torch.tensor(o.size(-1))))
            ...
            print('predictions:', iter_cnt)
            # dict = {'confidence': confidence_list, 'entropy': entropy_list}
            # np.save('experiment_data', dict)